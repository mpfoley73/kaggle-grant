...
This document addresses initial data management.
...
Assign all pre-2008 observations, and a random subset of 2008 observations to `training`, and the remainder to `testing`.

```{r}
pre2008 <- subset(d_raw_5, !is2008)
year2008 <- subset(d_raw_5, is2008)

set.seed(568)
inTrain <- createDataPartition(year2008$Class, p = 3/4)[[1]]
year2008_train <- year2008[ inTrain,]
testing   <- year2008[-inTrain,]
training <- rbind(pre2008, year2008_train)
...
rm(pre2008, year2008, year2008_train, inTrain)
```
...
high_cor_preds <- findCorrelation(all_cor, .99)
low_cor_preds <- all_preds[-high_cor_preds]

problematic_preds <- nearZeroVar(
  training[, low_cor_preds], 
  saveMetrics = TRUE, 
  freqCut = floor(nrow(training)/5))

fullSet <- rownames(subset(problematic_preds, !nzv))

reducedSet <- rownames(subset(problematic_preds, !nzv & 
                                freqRatio < floor(nrow(training)/50)))
...
rm(all_preds, all_cor, high_cor_preds, low_cor_preds, problematic_preds)
```
...
```{r}
save(training, testing, fullSet, reducedSet, file = "./grant_01.RData")
```
